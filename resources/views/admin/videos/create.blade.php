        ...
        <div class="row row-cards">
            <div class="col-12">
                <div class="card-header">
                    <h4 class="card-title"> دسته ویدئو</h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-xl-12">
                            @include('home.error')
                            <form action="{{route('video.store')}}" class="form-control" method="post" enctype="multipart/form-data">
                                @csrf
                                @method('post')
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" name="price" value="{{old('price')}}" class="form-control" name="example-text-input" placeholder="  قیمت ">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">ترتیب</label>
                                    <input type="number" name="sort" value="{{old('sort')}}" class="form-control" name="example-text-input" placeholder="ترتیب      ">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label" for="">دسته بندی</label>
                                    <select class="form-control" value="{{old('vcat_id')}}" name="vcat_id" id="vcat">
                                        <option selected disabled>دسته  بندی</option>
                                        @foreach (\App\Models\Vcat::all() as $vcat )
                                            <option  {{old('vcat_id')==$vcat->id?'selected':''}} value="{{$vcat->id}}">{{$vcat->title}}</option>
                                        @endforeach
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-control"  name="type" id="type">
                                        ...
                                        <option {{old('type')=='free'?'selected':''}} value="free">رایگان </option>
                                        <option {{old('type')=='money'?'selected':''}} value="money">پولی</option>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <select class="form-control"  name="model" id="model">
                                        ...
                                        <option {{old('model')=='close'?'selected':''}} value="close">بسته </option>
                                        <option {{old('model')=='open'?'selected':''}} value="open">باز</option>
                                    </select>
                                </div>
                                <div class="mb-3">
